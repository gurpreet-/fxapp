...
class ConversionRepositoryImplTest : BaseUnitTest() {

    private val apiRepository = mockk<ApiRepository>(relaxed = true)
    private lateinit var conversionRepository: ConversionRepositoryImpl
    ...
    fun beforeTest() {
        ...
        conversionRepository = ConversionRepositoryImpl(apiRepository)
    }

    @Test
    fun `filters GBP out when requesting GBP`() = runTest {
        val rates = conversionRepository.getExchangedRatesForAmountFormatted(oneGbp)
        assertFalse {
    ...
    fun `parsing malformed number in 0`() = runTest {
        val numberToTest = "234........."
        assertEquals("0", conversionRepository.tryParseAsNumber(numberToTest))
    }

    private fun testExtractsNumber(toTest: String, expected: String): String {
        val onlyNumber = conversionRepository.extractNumbersAndSeparator(toTest)
